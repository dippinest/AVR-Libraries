...
ISR(TIMER2_OVF_vect)
{
	SOFTPWM_Channel_Processing(&softpwm);
}
...
int main(void)
{
	...
	softpwm = SOFTPWM_Get_Channel_Object(T(PORTB), 1, MAX_PWM_WIDTH, MAX_PWM_WIDTH);
	...
	while (1)
	{
		...
		//
		while(SOFTPWM_DUTY_CYCLE(softpwm) != 0)
		{
			SOFTPWM_DUTY_CYCLE(softpwm)--;
			_delay_ms(DELAY_MS);
		}
		while(SOFTPWM_DUTY_CYCLE(softpwm) != MAX_PWM_WIDTH)
		{
			SOFTPWM_DUTY_CYCLE(softpwm)++;
			_delay_ms(DELAY_MS);
		}
