...
void Button_Long_Pressed_Polling(Button_t *button, uint16_t press_duration_counter, void (*button_callback)())
{
	...
	if (button->pull == BUTTON_INPUT_PULLUP)
	{
		if (_Bit_Is_Reset_P(button->input_pinx, button->input_pin))
		{
			if (button->status < press_duration_counter)
			{
				++(button->status);
			}
			else if (button->status == press_duration_counter)
			{
				button_callback();
				++(button->status);
			}
