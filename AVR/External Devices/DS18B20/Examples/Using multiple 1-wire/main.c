...
int main(void)
{
	...
	ONEWIRE_t onewire_1 = ONEWIRE_Get_Interface_Object(T(DDRA), T(PINA), 0);
	ONEWIRE_t onewire_2 = ONEWIRE_Get_Interface_Object(T(DDRA), T(PINA), 1);
	
	
	ONEWIRE_Set_Target_Interface_Object(&onewire_1);
	DS18B20_Set_Resolution(DS18B20_12BIT_RESOLUTION);
	
	ONEWIRE_Set_Target_Interface_Object(&onewire_2);
	DS18B20_Set_Resolution(DS18B20_12BIT_RESOLUTION);
