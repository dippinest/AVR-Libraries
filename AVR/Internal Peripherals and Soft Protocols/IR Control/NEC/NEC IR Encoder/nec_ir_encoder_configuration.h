...
#ifndef NEC_IR_ENCODER_CONFIGURATION_H_
#define NEC_IR_ENCODER_CONFIGURATION_H_
...
//
//
#define NEC_IR_ENCODER_IR_OUT_GPIO_DDR    DDRB
#define NEC_IR_ENCODER_IR_OUT_GPIO_PORT   PORTB
#define NEC_IR_ENCODER_IR_OUT_GPIO_PIN    1
...
//
#define NEC_IR_ENCODER_TIMER_VECTOR_INTERRUPT TIMER2_COMP_vect
...
//
inline void NEC_IR_Encoder_FSM_Timer_Interrupt_Initialize()
{
	TIMSK = (1 << OCIE2);
...
// (these settings are relevant for ATmega8A, 16 MHz)
//
inline void NEC_IR_Encoder_Carrier_Frequency_Generator_Timer_Initialize()
{
	TCNT1  = 0xFF22;
	OCR1A  = 0x00DD;
...
//
inline void _NEC_IR_Encoder_Set_IR_Out_Level_High()
{
	TCCR1A = (1 << COM1A0);
}

inline void _NEC_IR_Encoder_Set_IR_Out_Level_Low()
{
	TCCR1A = (1 << COM1A1);
