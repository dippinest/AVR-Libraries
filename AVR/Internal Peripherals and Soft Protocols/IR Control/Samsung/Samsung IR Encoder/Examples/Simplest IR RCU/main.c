...
// в файле samsung_ir_encoder_configuration.h !!!
//
// -------------------------------------------------------------------------------
//
// Attention!!! Don't forget to configure the library
// in the samsung_ir_encoder_configuration.h file!!!
//
// ===============================================================================
...
int main(void)
{
    ...
    while (1) 
    {
		...
		if ( !(PINC & (1 << 0)) )
		{
			if (button_flag == true)
			{
				Samsung_IR_Encoder_Transmit_Standart_Packet(0x12, 0x34, true);
		...
		{
			Samsung_IR_Encoder_Wait_Complete_And_Stop_Transmittion();
