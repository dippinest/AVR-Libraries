...
int main(void)
{
	...
	SOFTPWM_Initialize_Object(&(softpwm[0]), T(PORTB), 1, MAX_PWM_WIDTH, MAX_PWM_WIDTH);
	SOFTPWM_Initialize_Object(&(softpwm[1]), T(PORTB), 2, MAX_PWM_WIDTH, MAX_PWM_WIDTH);
	SOFTPWM_Initialize_Object(&(softpwm[2]), T(PORTB), 3, MAX_PWM_WIDTH, MAX_PWM_WIDTH);
	...
	while (1)
	{
		...
		//
		while(SOFTPWM_DUTY_CYCLE(softpwm[0]) != 0)
		{
			SOFTPWM_DUTY_CYCLE(softpwm[0])--;
			_delay_ms(DELAY_MS);
		}
		while(SOFTPWM_DUTY_CYCLE(softpwm[0]) != MAX_PWM_WIDTH)
		{
			SOFTPWM_DUTY_CYCLE(softpwm[0])++;
			_delay_ms(DELAY_MS);
		}
		
		while(SOFTPWM_DUTY_CYCLE(softpwm[1]) != 0)
		{
			SOFTPWM_DUTY_CYCLE(softpwm[1])--;
			_delay_ms(DELAY_MS);
		}
		while(SOFTPWM_DUTY_CYCLE(softpwm[1]) != MAX_PWM_WIDTH)
		{
			SOFTPWM_DUTY_CYCLE(softpwm[1])++;
			_delay_ms(DELAY_MS);
		}
		
		while(SOFTPWM_DUTY_CYCLE(softpwm[2]) != 0)
		{
			SOFTPWM_DUTY_CYCLE(softpwm[2])--;
			_delay_ms(DELAY_MS);
		}
		while(SOFTPWM_DUTY_CYCLE(softpwm[2]) != MAX_PWM_WIDTH)
		{
			SOFTPWM_DUTY_CYCLE(softpwm[2])++;
			_delay_ms(DELAY_MS);
		}
