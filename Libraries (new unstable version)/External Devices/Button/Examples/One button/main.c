...
int main(void)
{
	Button_Initialize_Object(&button, T(DDRD), T(PIND), 2);
	...
	while (1)
	{
		...
		//
		Button_PULLUP_Pressed_And_Released_Polling(&button, Press, Release);
		...
		// will reset to 0
		Button_PULLUP_Hold_With_Timeout_Polling(&button, 10, Hold_With_Timeout);
		...
		// В отличии от Button_PULLUP_Hold_With_Timeout_Polling() этот обработчик
		// выполняется однократно при зажатой кнопке
		// -------------------------------------------------------------------------------
		...
		// Unlike Button_PULLUP_Hold_With_Timeout_Polling(),
		// this handler is executed once when the button is clamped
		Button_PULLUP_Long_Pressed_Polling(&button, 50, Long_Press);
