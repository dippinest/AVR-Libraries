...
// LUT таблица переходов для опроса
// -------------------------------------------------------------------------------
// LUT lookup table for polling
//
...
void Encoder_Initialize(void (*left_turn_callback_function)(), void (*right_turn_callback_function)())
{
	ENCODER_INPUT0_DDR &= ~(1 << ENCODER_INPUT0_PIN);
	ENCODER_INPUT1_DDR &= ~(1 << ENCODER_INPUT1_PIN);
	
	_left_turn_callback  = left_turn_callback_function;
	_right_turn_callback = right_turn_callback_function;
}
...
void Encoder_Initialize_Object
(
	Encoder_t *encoder,
